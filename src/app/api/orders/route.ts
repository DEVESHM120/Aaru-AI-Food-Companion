...
export async function GET() {
  ...
  try {
    const response = await (client.beta.messages as any).create({
      ...
      max_tokens: 1024,
      system: "You are a helper. Call the get_past_orders tool and return the result as a JSON array. Each item must have: restaurantName, itemName, price (number in INR), platform ('zomato'), orderedAt (ISO date string). Return only the JSON array, no explanation.",
